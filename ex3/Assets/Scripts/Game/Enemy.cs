...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        GameObject nearestPlayer = GameManager.Instance.NearestPlayer(mRigidBody.transform.position); // finds the nearest player
        Vector3 playerPosition = new Vector3(nearestPlayer.transform.position.x, nearestPlayer.transform.position.y, nearestPlayer.transform.position.z);
        
        transform.LookAt(nearestPlayer.transform); // looks at the player
        transform.position = Vector3.MoveTowards(transform.position, playerPosition, speed * Time.deltaTime); // moves toward the player
