...
public class Gun : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (mFiring && fireEnabled)
        {
            ...
            var secondsPerBullet = (shotgun == 2 ? 12.0f : 1.0f) / (fireRate / 60.0f); // waits longer in case of bomb mode
    ...
    public void ShootGun(Transform director)
    {
        ...
        /*
         Debug.Log(director.rotation); //
        SpawnBullet(
            new Vector3{ x = 0.0f, y = 0.0f, z = 0.0f },
            Quaternion.Euler(0.0f, 0.0f, 0.0f)
        );*/ 

        // switching between gun modes
        float shotLines = 1.0f;
        switch (shotgun)
        {
            case 0: shotLines = 1.0f; break; // single shot
            case 1: shotLines = 6.0f; break; // shotgun
            case 2: shotLines = 72.0f; break; // bomb
        }
        float change;
        float angle = shotgunSpread / shotgunBullets;
        for (int i = 0; i < shotLines; i++)
        {
            change = ((-shotLines + 1) / 2 + i) * angle;
            SpawnBullet(director.position, Quaternion.Euler(director.eulerAngles.x, director.eulerAngles.y + change , director.eulerAngles.z));
        }
